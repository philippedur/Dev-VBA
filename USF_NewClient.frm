VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} USF_NewClient 
   Caption         =   "USF_NewClient"
   ClientHeight    =   7710
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   12465
   OleObjectBlob   =   "USF_NewClient.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "USF_NewClient"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False

Option Explicit    ' USERFORM UsF_NewClient !!!!!!!!!!!!!!!!!!!
...
Dim TB()
Dim txt() As New classe_textb_Usf_newclient
Dim Cmb() As New classe_combo_Usf_newclient
...
Public Sub Userform_initialize()    '///// PAS DE CHECK DE CELLULES VIDES
    ...
    USF_NewClient.BackColor = GetLongFromRGB(160, 255, 64)
    USF_NewClient.Label1.BackColor = GetLongFromRGB(160, 255, 64)
    USF_NewClient.Label2.BackColor = GetLongFromRGB(160, 255, 64)
    ...
    '        Me("TextBox1").ForeColor = &H80000011
        ...
        Me("TextBox1").Top = Y
        Me("TextBox1").Left = X
        Me("TextBox1").Width = 120
        Me("TextBox1").Visible = True
        Me("TextBox1").SpecialEffect = 0
    End With

    Set R_C2 = Me.Controls.Add("Forms.TextBox.1", "TextBox2", True)
    With R_C2
    '        ' Couleur = C1.Cells(ligne, i).Interior.Color
    '        Me("TextBox2").BackColor = Couleur
        Me("TextBox2").Value = ""    ' c3.Cells(ligne, 14)
        Me("TextBox2").FontSize = 10
        Me("TextBox2").Top = Y
        Me("TextBox2").Left = X + 125
    ...
    Set R_C20 = Me.Controls.Add("Forms.ComboBox.1", "ComboBox20", True)
    With R_C20
    ' Couleur = c3.Cells(ligne, i).Interior.Color
    '        Me("Combobox20").BackColor = Couleur
        Me("Combobox20").Value = ""    ' c3.Cells(ligne, 18).Value
        Me("Combobox20").FontSize = 10
        Me("Combobox20").Top = Y
        Me("Combobox20").Left = X + 218
        Me("Combobox20").Width = 100
        Me("Combobox20").Visible = True
        Me("Combobox20").SpecialEffect = 0
        For L = 2 To c5.Range("D" & Rows.Count).End(xlUp).Row
            Me("ComboBox20").AddItem Format(c5.Range("D" & Trim(Str(L))), "###0.00 €")
        Next
    End With
...
Public Sub affich_raz()
    ...
    Me("Combobox20").Value = ""    'c3.Cells(ligne, 9).Value
...
Public Sub affiche()
    ligne = pos
    Me("TextBox1").Value = c3.Cells(ligne, 6).Value        ' Gerant
    Me("TextBox2").Value = c3.Cells(ligne, 14).Value       ' Societe
    ...
    Me("Combobox20").Value = c3.Cells(ligne, 19).Value       ' Typ_DOM
    Me("Textbox22").Value = c3.Cells(ligne, 9).Value        ' SIREN/SIRET
    Me("Textbox24").Value = c3.Cells(ligne, 12).Value
    Me("Textbox26").Value = c3.Cells(ligne, 13).Value
    Me("Combobox3").Value = c3.Cells(ligne, 24).Value       ' périodicité
    Me("Combobox21").Value = c3.Cells(ligne, 8).Value       ' Typ_DOM
End Sub
Public Sub Valid_Click()
    If Me("Combobox3").Value <> "" And Me("Combobox21").Value <> "" Then
    Set c3 = Sheets("CLIENTS")
    Call tri_col_generic(Sheets("CLIENTS"), 7)
    r1 = c3.Range("G65000").End(xlUp).Row
    r2 = c3.Range("G" & r1)
    Call tri_col_generic(Sheets("CLIENTS"), 14)
    nbrowmax = c3.Range("G65000").End(xlUp).Row
    ligne = nbrowmax + 1
    c3.Cells(ligne, 6).Value = Me("TextBox1").Value       ' Gerant
    c3.Cells(ligne, 14).Value = Me("TextBox2").Value      ' Societe
'    c3.Cells(ligne, 4).Value = ttk_Date(Date)  ' Format(Date, "DD/MM/YYYY")  ' Date création Societe
    c3.Cells(ligne, 4).Value = Format(ttk_Date(Date), "DD/MM/YYYY")  ' Date création Societe
    c3.Cells(ligne, 7).Value = r2 + 1                      ' Numero client
    ...
    c3.Cells(ligne, 19).Value = Me("Combobox20").Value      ' Typ_DOM
    ...
    Call Addrep_Onerec(c3.Cells(ligne, 14).Value, ligne)
    USF_Client_edit.Repaint
        ElseIf Me("Combobox3").Value <> "" Then
    MsgBox ("Saisissez une périodicité de facturation.")
        ElseIf Me("Combobox21").Value <> "" Then
    MsgBox ("Saisissez une périodicité de facturation.")
    End If
End Sub

Private Function decod6(InputValue)
    With InputValue
    sstr1 = InStr(1, InputValue, ") -") + 2
    sstr2 = Right(InputValue, Len(InputValue) - sstr1)
    decod6 = sstr2
    End With
End Function
