VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} USF_Client 
   Caption         =   "Compte_client"
   ClientHeight    =   6810
   ClientLeft      =   120
   ClientTop       =   450
   ClientWidth     =   12465
   OleObjectBlob   =   "USF_Client.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "USF_Client"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit    ' USERFORM UsF_client !!!!!!!!!!!!!!!!!!!
...
Dim TB()
Dim txt() As New classe_textb_Usf_client
Dim Cmb() As New classe_combo_Usf_client
...
Public Sub Userform_initialize()    '///// PAS DE CHECK DE CELLULES VIDES
    ...
    USF_Client.BackColor = GetLongFromRGB(255, 192, 255)
    nbrowmax = c3.Range("B65000").End(xlUp).Row
    Application.EnableEvents = True
    If new_row = True Then
'    USF_Client.Valid.Enabled = True
    Call affiche_raz
    Else
'    USF_Client.Valid.Enabled = False
    End If
    ...
    '        Me("Textbox28").BackColor = Couleur
        Me("Textbox28").Value = c3.Cells(ligne, 24).Value
        Me("Textbox28").FontSize = 10
        Me("Textbox28").Top = Y
        Me("Textbox28").Left = X + 60
        Me("Textbox28").Width = 50
        Me("Textbox28").Visible = True
        End With
        
        Set R_t29 = Me.Controls.Add("Forms.TextBox.1", "Textbox29", True)
    With R_t29
    ' Couleur = c3.Cells(ligne, i).Interior.Color
    '        Me("Combobox20").BackColor = Couleur
        Me("TextBox29").Value = c3.Cells(ligne, 8).Value
        Me("TextBox29").FontSize = 10
        Me("TextBox29").Top = Y
        Me("TextBox29").Left = X + 88
        Me("TextBox29").Width = 100
        Me("TextBox29").Visible = True
    End With
    Y = Y + 25
    X = 3

    Set L_t2 = Me.Controls.Add("Forms.label.1", "label2", True)
    With L_t2
    ' Couleur = c3.Cells(ligne, i).Interior.Color
    '        Me("label29").BackColor = Couleur
        Me("label2").Caption = "Infos juridiques: "
        Me("label2").FontSize = 10
'        Me("label2").Font.Bold = True
        Me("label2").Height = 12
...
Public Sub affiche()
    ligne = pos
    Me("ComboBox1").Value = c3.Cells(ligne, 6).Value        ' Societe
    ...
    Me("Textbox20").Value = c3.Cells(ligne, 20).Value       ' Typ_DOM
    Me("Textbox22").Value = c3.Cells(ligne, 9).Value        ' SIREN/SIRET
    Me("Textbox24").Value = c3.Cells(ligne, 12).Value
    Me("Textbox26").Value = c3.Cells(ligne, 13).Value
    Me("Textbox28").Value = c3.Cells(ligne, 24).Value       ' périodicité
    Me("Textbox29").Value = c3.Cells(ligne, 8).Value       ' H/F
    Me("Label1").Caption = c3.Cells(ligne, 25).Value       ' hyperlink
End Sub
Public Sub Label1_Click()
   With Me("Label1")
        sstr1 = c3.Cells(ligne, 25).Hyperlinks(1).Address
        If sstr1 <> "" Then
         .MouseIcon = LoadPicture("C:\WINDOWS\Cursors\aero_link.cur")  'A vérifier si le fichier "Cursor" est dispo
        strAddress = c3.Cells(ligne, 25).text
        Me("label1").Caption = strAddress
        .ForeColor = &HFF0000
        .Font.Underline = True
        .MousePointer = fmMousePointerCustom
        strAddress = c3.Cells(ligne, 25).text
        ThisWorkbook.FollowHyperlink Address:=sstr1, NewWindow:=True
        sstr1 = c3.Cells(ligne, 25).Hyperlinks(1).Address
        .MouseIcon = LoadPicture("C:\WINDOWS\Cursors\aero_link.cur")  'A vérifier si le fichier "Cursor" est dispo
        [/COLOR]
    End If
...
Public Sub Valid_Click()
    ...
    c3.Cells(ligne, 4).Value = Format(Date, "DDMMYYYY")    ' Date création Societe
    c3.Cells(ligne, 7).Value = nbrowmax + 1                ' Numero client
    c3.Cells(ligne, 3).Value = Me("Textbox8").Value        ' Ville
    c3.Cells(ligne, 3).Value = Me("Textbox10").Value       ' Gérant
    c3.Cells(ligne, 21).Value = Me("Textbox12").dValue     ' Mail
    c3.Cells(ligne, 22).Value = Me("Textbox14").Value      ' Tel1
    c3.Cells(ligne, 23).Value = Me("Textbox16").Value      ' Tel2
    ...
    c3.Cells(ligne, 24).Value = Me("Textbox28").Value      'periodicité
    c3.Cells(ligne, 8).Value = Me("Combobox20").Value      'sexe
    USF_Client.Repaint
End Sub
